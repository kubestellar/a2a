...
class ClusterManagementFunction(BaseFunction):
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if operation == "list":
                ...
                if not cluster_name:
                    return {"status": "error", "error": "cluster_name is required for register operation"}
                return await self._register_cluster(cluster_name, context, labels or {}, kubeconfig)
            elif operation == "label":
                if not cluster_name:
                    return {"status": "error", "error": "cluster_name is required for label operation"}
                if not labels:
                    return {"status": "error", "error": "labels are required for label operation"}
                return await self._label_cluster(cluster_name, context, labels, kubeconfig)
            elif operation == "unregister":
                if not cluster_name:
                    return {"status": "error", "error": "cluster_name is required for unregister operation"}
                return await self._unregister_cluster(cluster_name, context, kubeconfig)
            elif operation == "update-labels":
                if not cluster_name:
                    return {"status": "error", "error": "cluster_name is required for update-labels operation"}
                if not labels:
                    return {"status": "error", "error": "labels are required for update-labels operation"}
                return await self._update_labels(cluster_name, context, labels, kubeconfig)
            else:
    ...
    async def _list_clusters(self, context: str, kubeconfig: str) -> Dict[str, Any]:
        ...
        if "its" in context.lower():
            cmd = ["kubectl", "--context", context, "get", "managedclusters", "-o", "json"]
        else:
    ...
    async def _register_cluster(self, cluster_name: str, context: str, labels: Dict[str, str], kubeconfig: str) -> Dict[str, Any]:
        ...
        if list_result["status"] == "success":
            for cluster in list_result["clusters"]:
                if cluster["name"] == cluster_name:
                    return {"status": "error", "error": f"Cluster '{cluster_name}' is already registered"}
        ...
        binding_manifest = {
            ...
            "spec": {
                "destinations": [{"clusterId": cluster_name}],
                "workload": {}
            }
        }
        ...
        ret = await run_shell_command_with_cancellation(cmd, input_data=binding_yaml.encode())
        if ret["returncode"] != 0:
            return {"status": "error", "error": ret["stderr"]}
    ...
    async def _label_cluster(self, cluster_name: str, context: str, labels: Dict[str, str], kubeconfig: str) -> Dict[str, Any]:
        """Apply labels to a registered cluster."""
        # Update the binding with new labels
        cmd = ["kubectl", "--context", context, "label", "bindings.control.kubestellar.io", f"{cluster_name}-binding"]
    ...
    async def _update_labels(self, cluster_name: str, context: str, labels: Dict[str, str], kubeconfig: str) -> Dict[str, Any]:
        """Update labels on a registered cluster."""
        # First, get current labels
        cmd = ["kubectl", "--context", context, "get", "bindings.control.kubestellar.io", f"{cluster_name}-binding", "-o", "json"]
        if kubeconfig:
            cmd += ["--kubeconfig", kubeconfig]
    ...
    async def _unregister_cluster(self, cluster_name: str, context: str, kubeconfig: str) -> Dict[str, Any]:
        """Remove a cluster from the WDS context."""
        binding_name = f"{cluster_name}-binding"
        cmd = ["kubectl", "--context", context, "delete", "bindings.control.kubestellar.io", binding_name]
        if kubeconfig:
            cmd += ["--kubeconfig", kubeconfig]
    ...
    async def _apply_cluster_labels(self, cluster_name: str, labels: Dict[str, str], kubeconfig: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                "operation": {
                    ...
                    "enum": ["list", "register", "label", "unregister", "update-labels"],
                    "default": "list"
                },
                "cluster_name": {
                    "type": "string",
